...
type Appointment = { id: string; title?: string; name?: string; leadName?: string; email?: string; phone?: string; startsAt?: string; endsAt?: string; status?: string; selectedOption?: string; serviceNeeded?: string; requirement?: string; requirements?: string; message?: string };
...
export default function SchedulePage() {
  ...
  const sorted = useMemo(() => [...appointments].sort((a, b) => new Date(a.startsAt || 0).getTime() - new Date(b.startsAt || 0).getTime()), [appointments]);
  const upcoming = sorted.filter((item) => !item.startsAt || new Date(item.startsAt).getTime() >= Date.now());
  const old = sorted.filter((item) => item.startsAt && new Date(item.startsAt).getTime() < Date.now()).reverse();
...
function AppointmentCard({ appointment, index, compact = false }: { appointment: Appointment; index: number; compact?: boolean }) {
  ...
  const message = appointment.requirement || appointment.requirements || appointment.message || "No requirement added.";
  return (
    <motion.div
    ...
    >
      <div className="grid gap-4 md:grid-cols-[180px_1fr_auto] md:items-center">
        <div>
          ...
          <p className="mt-1 text-xs font-semibold text-white/35">{duration(appointment.startsAt, appointment.endsAt)}</p>
        </div>
        <div>
          ...
          <p className="mt-1 text-sm font-semibold text-white/45"><UsersRound className="mr-2 inline h-4 w-4" />{appointment.leadName || appointment.name || "Lead"} {appointment.phone ? `· ${appointment.phone}` : ""}</p>
